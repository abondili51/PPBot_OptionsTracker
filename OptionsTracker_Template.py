...
class mSettings():
    ...
    SweepsRegex: str = r'^(?P<here>@\w+)\s*(?P<ticker>\[\w+\])\s*(?P<optional>\w* OPTION ALERT:)*\s*(?P<month>\w+)\s*((?P<date>\d+,*\s*\d*))*\s*\$(?P<strike>\d+\.*\d*)\s*(?P<type>\w+).*\s*(?P<ask>ASK!*:)\s*(?P<quantity>\d+)\s*@\s*\$\s*(?P<price>\d+\.*\d*)\s*.*'	
